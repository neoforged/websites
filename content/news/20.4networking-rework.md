...
# Introduction
## Reworked networking
Welcome to the reworked networking blog post for NeoForge.
...
It is as such of the upmost importance that you return the same value, as in the same resource location, from the `id()` method on the payload instance, as well as give to the registrar during registration. 
