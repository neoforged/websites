...
The packet type and stream codec must be registered using the `RegisterPayloadHandlerEvent` event.
...
```java
    public static void onPayloadRegister(RegisterPayloadHandlerEvent event) {
        ...
        registrar.play(TwoStacksPayload.TYPE, TwoStacksPayload.STREAM_CODEC, <packet handler>);
        // and so on...
    }
```

### Network Refactor? (TODO)
